...
import com.youdian.jcompz.MapPz;
...
public class newBookCheckIFrame extends JInternalFrame {
	...
	private Object[][] getFileStates(List list){
		...
		for(int i=0;i<list.size();i++){
			...
			results[i][2]=order.getDgnumber();
			results[i][3]=order.getOperator();
			
			String CheckAndAccepts;
			if(order.getCheckAndAccept() == 1 ){
				//1代表没有验收
				CheckAndAccepts="否";
			}else{
				CheckAndAccepts="是";
			...
			results[i][5]=order.getDiscount();
			
			String bookTypes=String.valueOf(MapPz.getMap().get(String.valueOf(order.getTypeId())));
			results[i][6]=bookTypes;
			
			results[i][7]=order.getBookName();
			results[i][8]=order.getWriter();
			results[i][9]=order.getTranslator();
			results[i][10]=order.getPublisher();
			results[i][11]=order.getPublish_date();
	...
	class TableListener extends MouseAdapter {
		public void mouseClicked(final MouseEvent e) {
			...
			Double x=Double.valueOf(table.getValueAt(selRow, 12).toString().trim());
			Double y=Double.valueOf(table.getValueAt(selRow, 2).toString().trim());
			Double z=Double.valueOf(table.getValueAt(selRow, 5).toString().trim());
			orderPrice.setText(x.doubleValue()*y.doubleValue()*z.doubleValue()/10+"");
