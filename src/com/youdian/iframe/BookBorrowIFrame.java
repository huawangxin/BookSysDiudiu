...
//		});//关闭窗口时候引发的事件
			...
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				if (readerISBN.getText().trim().length()!=0
					...
					for (int i = 0; i < list.size(); i++) {
						...
						bookType.setText(String.valueOf(map.get(String.valueOf(book
								.getTypeId()))));
		...
		public void keyTyped(KeyEvent e) {
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				...
				for (int i = 0; i < list.size(); i++) {
					...
					number.setText(String.valueOf(reader.getMaxborrow()));
	...
	class BorrowActionListener implements ActionListener { 
		public void actionPerformed(final ActionEvent e) {
			...
			//System.out.println(java.sql.Timestamp.valueOf(borrowDate));
//			int i=Dao.InsertBookBorrow(bookISBNs, readerISBNs ,operatorId, borrowDate, backDate);
			int i=Dao.InsertBookBorrow(bookISBNs, readerISBNs, operatorId, borrowDate, backDate);
