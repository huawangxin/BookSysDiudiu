...
public class BookTypeAddIFrame extends JInternalFrame {
	...
	public BookTypeAddIFrame() {
		...
		fine = new JTextField("单位为角");
		fine.setColumns(30);
		fine.addKeyListener(new NumberListener());
		...
		button.addActionListener(new ActionListener(){
			public void actionPerformed(final ActionEvent e) {
				...
				if(fine.getText().length()==0||fine.getText().trim().equals("单位为角")){
					JOptionPane.showMessageDialog(null, "罚款文本框不可为空");
					return;
				}
				int i=Dao.InsertBookType(bookTypeName.getText().trim(), Integer.parseInt(days.getText().trim()),fine.getText().trim());
				if(i==1){
