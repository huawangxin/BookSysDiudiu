...
public class newBookOrderIFrame extends JInternalFrame {
	...
	public newBookOrderIFrame() {
		...
		for(int i=0;i<list.size();i++){
			...
			item.setName((String)booktype.getType());
	...
	class ButtonAddLisenter implements ActionListener{
		public void actionPerformed(final ActionEvent e) {
			...
			String zks = zk.getText();
			try{
				int i=Dao.InsertBookOrder(ISBN.getText().trim(), orderDate.getText().trim(), Integer.parseInt(orderNumber.getText().trim()), operator.getText().trim(), checkAndAccept,zks);
		...
		public void keyTyped(KeyEvent e) {
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				...
				for (int i = 0; i < list.size(); i++) {
					...
					bookType.setSelectedItem(map.get(String.valueOf(bookinfo.getTypeId())));
